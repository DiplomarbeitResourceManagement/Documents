...
fp=open("L:\GitHub\Documents\Code\Absolventen\Absolventen_all_format_sozial.csv", "r")
fp1=codecs.open("L:\GitHub\Documents\Code\Absolventen\Absolventen_all_tables_v03.sql", "w+", encoding = 'utf8')	# sql file in utf8 speichern
...
for row in reader:
   if i != 0:
	   ...
	   xing_p.append(row[22])
	   fb_p.append(row[23])
	   linkedin_p.append(row[24])
   i=1
...
xing_p_u=unique_list(xing_p)
fb_p_u=unique_list(fb_p)
linkedin_p_u=unique_list(linkedin_p)
...
for elem in xing_p_u:
  fp1.write("insert into xing_profil (xing_p) values (\"" \
		+ elem + "\");\n" \
	   ) 
fp1.write("\n")

for elem in fb_p_u:
  fp1.write("insert into fb_profil (fb_p) values (\"" \
		+ elem + "\");\n" \
	   ) 
fp1.write("\n")

for elem in linkedin_p_u:
  fp1.write("insert into linkedin_profil (linkedin_p) values (\"" \
		+ elem + "\");\n" \
	   ) 
fp1.write("\n")
